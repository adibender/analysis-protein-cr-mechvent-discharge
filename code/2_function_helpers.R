...
make_six_frames <- function(
  ...
  formula1 =  Surv(event, PatientDied) ~ Year + DiagID2 + AdmCatID + Gender +
        ApacheIIScore + BMI + Propofol2_4 + inMV2_4 + OralIntake2_4 + PN2_4 + Age +
        CombinedicuID + icuByDummy,
  formula2 = Surv(event, PatientDischarged) ~ Year + DiagID2 + AdmCatID + Gender +
                          ApacheIIScore + BMI + Propofol2_4 + inMV2_4 + OralIntake2_4 + PN2_4 + Age +
                          CombinedicuID + icuByDummy) {
...
prettify_labels <- function(coeftab) {
  ...
  labels <- c(
    ...
    "Gender: male",
    "# of days with MV\n (up to day 4)",
    "# of days with propofol\n (up to day 4)",
    "# of days with oral intake\n (up to day 4)",
    "# of days with PN\n (up to day 4)",
    "Apache II Score\n (interaction with t)")
...
gg_4_panel <- function(mod) {
  ...
  p_baseline <- ggplot(
    ...
    geom_line(aes(y = ci_upper), lty = 2) + xlab("Days after ICU admission") + ylab("Ln (hazard rate)") +
  ...
  p_age <- ggplot(
    ...
    geom_line() +
    geom_line(aes(y = ci_lower), lty = 2) +
    geom_line(aes(y = ci_upper), lty = 2) + xlab("Years") + ylab("Ln (hazard rate)") +
    ggtitle("Age")
  ...
  p_bmi <- ggplot(
    ...
    geom_line() +
    geom_line(aes(y = ci_lower), lty = 2) +
    geom_line(aes(y = ci_upper), lty = 2) + xlab(TeX("kg/$m^2$")) + ylab("Ln (hazard rate)") +
    ggtitle("BMI")
