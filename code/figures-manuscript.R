...
m_2_A   <- readRDS(paste0(main_subfolder, "/models/m_2_A.Rds"))
m_2_B   <- readRDS(paste0(main_subfolder, "/models/m_2_B.Rds"))
...
saveRDS(comparisons_df, paste0(main_subfolder, "/results/comparisons_df.Rds"))
names(f6) <- names(f6_extubation) <-
  paste0(comparisons_df$p1.name, " vs. ", comparisons_df$p2.name)
saveRDS(f6, paste0(main_subfolder, "/models/m_2_A_6frames.Rds"))
saveRDS(f6_extubation, paste0(main_subfolder, "/models/m_2_B_6frames.Rds"))
...
saveRDS(list_p_scheme, paste0(main_subfolder, "/data/list_p_scheme.Rds"))
...
p_1
ggsave(p_1, file = paste0(main_subfolder, "/results/figures/protein_effect_comparisons_1.png"), height = 9, width = 9)
...
p_2 <- list_p_scheme[[4]] + xlab("Days on diet") + ylab("g protein / kg day") +
  ...
  plot_layout(nrow = 3, byrow = TRUE)
p_2
ggsave(p_2, file = paste0(main_subfolder, "/results/figures/protein_effect_comparisons_2.png"), height = 9, width = 9)
